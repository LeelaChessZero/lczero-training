...
def extract_inputs_outputs(raw):
    ...
    input_format = tf.io.decode_raw(tf.strings.substr(raw, 4, 4), tf.int32)
    ...
    unit_planes = tf.where(input_format == 2, tf.concat([tf.concat([bitsplat_unit_planes[:, :1, :1], zero_plane[:, :, :6], bitsplat_unit_planes[:, 2:3, :1]], 2), tf.concat([bitsplat_unit_planes[:, 1:2, :1], zero_plane[:, :, :6], bitsplat_unit_planes[:, 3:4, :1]], 2), zero_plane, zero_plane, unit_planes[:,4:]], 1), unit_planes)
