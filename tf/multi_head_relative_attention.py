...
class MultiHeadRelativeAttention(Layer):
  """MultiHeadRelativeAttention layer.
  ...
  def __init__(self,
               ...
               **kwargs):
    super(MultiHeadRelativeAttention, self).__init__(**kwargs)
  ...
  def get_config(self):
    ...
    base_config = super(MultiHeadRelativeAttention, self).get_config()
  ...
  def _build_from_signature(self, query, value, key=None):
    ...
    with tf_utils.maybe_init_scope(self):
      ...
      height = self._query_shape[-3]
      width = self._query_shape[-2]
      self._height = height
      self._width = width
      ...
      coords = tf.reshape(tf.stack(coords), [2, -1])
      relative_coords = coords[:, :, None] - coords[:, None, :]

      relative_coords = tf.add(relative_coords, [[[height - 1]],[[width -1]]])
      relative_coords = tf.multiply(relative_coords, [[[2 * width -1]],[[1]]])
      relative_coords = tf.transpose(relative_coords, [1, 2, 0])
      self._relative_index = tf.reshape(tf.reduce_sum(relative_coords, axis=-1), [height*width,height*width])
  ...
  def _compute_attention(self,
    ...
    relative_bias = tf.gather(self._relative_bias_table, axis=0, indices=self._relative_index)
    relative_bias = tf.reshape(tf.transpose(relative_bias, [2, 0, 1]), [self._num_heads, self._height, self._width, self._height, self._width])
